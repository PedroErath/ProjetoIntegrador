                ...
                this.Conectar().then((db) => {      
                    db.transaction((tx) => {     
                        ...
                        for (let i = 0; i < len; i++) {            
                            ...
                            const {id, nome, nota, opiniao} = row;
                            lista.push({id, nome, nota, opiniao});
                        }
                        console.log(lista);          
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {    
            this.Conectar().then((db) => {      
                db.transaction((tx) => {     
                    ...
                    tx.executeSql('INSERT INTO Item (nome, nota, opiniao) VALUES (?, ?, ?)', [item.nome, item.nota, item.opiniao]).then(([tx, results]) => { 
                        resolve(results);        
                    });      
                }).then((result) => {        
                    this.Desconectar(db);      
